...
public class EmailValidation {
	 ...
	 * @Author: Anindita
	 * 
	 * @Description: Waits for the forgot password mail for AGS and clicks on the
	 * reset password link
	 */
	public static void forgotPasswordMailForAGS(WebDriver driver, String SS_path, app_AGS_Repo AGS) throws IOException {
		try {
			...
			if (flag == 1) {
				...
				AGS.enterConfirmPasswordInResetPassword(
						excelOperation.getTestData("TC05", "AGS_Test_Data", "Password"));
	 ...
	 * @Author: Anindita
	 * 
	 * @Description: Waits for the forgot password mail for VET and clicks on the
	 * reset password link
	 */
			...
			if (flag == 1) {
				...
				try {
					wait.until(ExpectedConditions
							.elementToBeClickable(driver.findElement(By.xpath("(//main[@class='yscrollbar']"
					...
					String ChildWindow = iterator.next();
					driver.switchTo().window(ChildWindow);
					VET.enterNewPassordInForgotPasswordPage(
							excelOperation.getTestData("TC04", "VET_Test_Data", "Password"));
					VET.enterConfirmPassordInForgotPasswordPage(
							excelOperation.getTestData("TC04", "VET_Test_Data", "Password"));
	 ...
	 */
	/*
	 * public static int forgotPasswordEmailForWEL(WebDriver driver, String SS_path,
	 * app_WEL_Repo WEL) throws IOException { try { WebDriverWait wait1 = new
	 * WebDriverWait(driver, Duration.ofSeconds(5)); int timeOutSeconds=60; int
	 * flag=0; WebElement element1 =
	 * driver.findElement(By.xpath("//button[@id='refresh']")); WebElement element2
	 * = null;
	 * 
	 * The purpose of this loop is to wait for maximum of 60 seconds for (int i = 0;
	 * i < timeOutSeconds / 5; i++) {
	 * 
	 * try { driver.switchTo().frame("ifinbox");
	 * element2=wait1.until(ExpectedConditions.visibilityOfElementLocated(By.
	 * xpath("//div[contains(text(),'Password Reset Request')]")));
	 * 
	 * if(element2.isDisplayed()==true) { flag=1; element2.click();
	 * driver.switchTo().defaultContent(); break; }
	 * 
	 * } catch (Exception e) { driver.switchTo().defaultContent(); element1.click();
	 * } }
	 * 
	 * if(flag==1) { driver.switchTo().frame("ifmail");
	 ...
	 * java.util.Iterator<String> iterator = allWindowHandles.iterator();
	 * 
	 * String yopmailHandle = iterator.next(); String ChildWindow=iterator.next();
	 * driver.switchTo().window(yopmailHandle); driver.close();
	 * driver.switchTo().window(ChildWindow); return flag; } else { Reporting.
	 * updateTestReport("No reset password mail was recieved in yopmail inbox",
	 * CaptureScreenshot.getScreenshot(SS_path), StatusDetails.FAIL); return flag; }
	 * } catch(Exception e) {
	 * Reporting.updateTestReport("Forgot password mail couldn't be validated",
	 * CaptureScreenshot.getScreenshot(SS_path), StatusDetails.FAIL); return 0; } }
	 */
