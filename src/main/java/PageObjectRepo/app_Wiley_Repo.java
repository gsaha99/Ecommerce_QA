...
public class app_Wiley_Repo {
	...
	@FindBy(xpath="(//div[@id='loginPageHeading'])[1]")
	...
	@FindBy(xpath="//div[@class='col-xs-7 typeOfProductCol']/input[@checked]/following-sibling::i")
	...
	public void enterExistingWileyUserPassword(String existingWileyUserPassword) throws IOException {
		try {
			ExistingWileyUserPassword.clear();
	...
	//Old Search Result page methods
		 ...
		 * @Description: Fetches the product's price from PDP
		 */
		public String fetchPriceInPDPInCAD() throws IOException {
			try {
				Reporting.updateTestReport(
						"Price of the product in PDP: "+ProductPriceInPDP.getText().trim()+" was returned",
						CaptureScreenshot.getScreenshot(SS_path), StatusDetails.PASS);
				return ProductPriceInPDP.getText().trim().split(" ")[1];
			}
			catch(Exception e) {
				Reporting.updateTestReport(
						"Price of the product in PDP could not be returned",
